...
MethodInvocation::MethodInvocation(Expression *expr, std::string name,
                                   ExpressionList *args, yy::location loc)
    : class_expr(expr), method_name(std::move(name)), arguments(args),
      Statement(loc), Expression(loc)
{
}

std::variant<int, std::string> MethodInvocation::eval() const { return 0; }
...
Type MethodInvocation::EvalType(ScopeLayer *scope)
{
    ...
    if (class_ref == nullptr)
    {
        ...
        throw std::runtime_error("No class '" + class_name.type_name +
                                 "' in scope");
    }
        ...
        (arguments != nullptr && arguments->expressions.size() != method_ref->arguments.size()))
    {
        ...
        throw std::runtime_error("Method '" + method_name + "' of class '" +
                                 class_name.type_name + "' expect " +
                                 std::to_string(method_ref->arguments.size()) +
                                 " arguments but got " +
    ...
    {
        // TODO inheritance
        if (arguments->expressions[i]->GetType(scope) !=
            method_ref->arguments[i].type)
        {
            std::cerr << arguments->expressions[i]->loc << std::endl;
            throw std::runtime_error(
                "Method '" + method_name + "' of class '" +
                class_name.type_name + "' expect at " + std::to_string(i) +
                "-th argument type '" +
