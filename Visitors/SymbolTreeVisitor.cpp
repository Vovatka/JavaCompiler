...
{
    if (statement->var_name->GetType(current_layer_) !=
        statement->new_value->GetType(current_layer_))
    {
        std::cerr << statement->loc << std::endl;
        throw std::runtime_error(
            "Assigment types isn't equal types '" +
            statement->var_name->GetType(current_layer_).ToString() +
            "' and '" +
            statement->new_value->GetType(current_layer_).ToString() + "'");
    }
}
...
{
    current_layer_->DeclareLocalVariable(
        STVariable(formal->name, formal->type));
}
...
void SymbolTreeVisitor::Visit(Print *statement)
{
        ...
        statement->expr->GetType(current_layer_) != Type("bool"))
    {
        std::cerr << statement->loc << std::endl;
        throw std::runtime_error(
            "Can print only 'int' or 'bool'. not '" +
            statement->expr->GetType(current_layer_).ToString() + "'");
    }
}
...
void SymbolTreeVisitor::Visit(Return *statement)
{
    ...
    if (method_ptr->return_type != statement->expr->GetType(current_layer_))
    {
        std::cerr << statement->expr->loc << std::endl;
        throw std::runtime_error(
            "'return' in method '" + method_ptr->GetName() + "' expect '" +
            method_ptr->return_type.ToString() + "' but got '" +
            statement->expr->GetType(current_layer_).ToString() + "'");
    }
}
