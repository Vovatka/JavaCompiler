...
std::pair<int, int> GetPairOfInts(BinaryExpression *expression,
...
std::pair<bool, bool> GetPairOfBools(BinaryExpression *expression,
                                     ExecuteVisitor *visitor)
{
...
PhysicalVariable::PtrType ExecuteVisitor::GetRes() const { return rax_; }
...
void ExecuteVisitor::Visit(MethodInvocation *statement)
{
    ...
    statement->GetScope()
        ->GetClassByName(type.type_name)
        ->FindMethodByName(statement->method_name)
...
void ExecuteVisitor::Visit(IdentExpression *expression)
{
    size_t ind =
        frame_pointer.top() +
        expression->GetScope()->GetOffsetOfVariable(expression->var_name);
    rax_ = stack_[ind];
}
