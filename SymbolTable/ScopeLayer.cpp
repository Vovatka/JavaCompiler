...
ScopeLayer::ScopeLayer(ScopeLayer *parent)
    : parent_(parent), current_offset_(parent->GetCurrentOffset())
{
    if (parent_ != nullptr)
        parent_->AddChild(this);
}

ScopeLayer::ScopeLayer(ScopeLayer *parent, long long int new_offset)
    : parent_(parent), current_offset_(new_offset)
{
    if (parent_ != nullptr)
        parent->AddChild(this);
}

ScopeLayer::ScopeLayer() : parent_(nullptr), current_offset_(0) {}
...
long long ScopeLayer::GetCurrentOffset() const { return current_offset_; }
...
void ScopeLayer::DeclareLocalVariable(const STVariable &var)
{
    ...
    {
        throw std::runtime_error("Variable '" + var.GetName() +
                                 "' has been declared in this scope");
    }
    if (current_offset_ == THIS_OFFSET)
        current_offset_ = 0;

    variables_.emplace(var.GetName(), var);
    std::cout << "Local variable '" << var.GetName() << "' in method '"
              << (current_method_ == nullptr ? "main"
                                             : current_method_->GetName())
              << "' of type '" << var.type.ToString()
              << "' has offset: " << current_offset_ << std::endl;
