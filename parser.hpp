...
/**
 ** \file /home/wowatka/workdir/MRA/JavaCompiler/parser.hpp
...
#ifndef YY_YY_HOME_WOWATKA_WORKDIR_MRA_JAVACOMPILER_PARSER_HPP_INCLUDED
# define YY_YY_HOME_WOWATKA_WORKDIR_MRA_JAVACOMPILER_PARSER_HPP_INCLUDED
// "%code requires" blocks.
#line 11 "/home/wowatka/workdir/MRA/JavaCompiler/parser.y"
...
#line 58 "/home/wowatka/workdir/MRA/JavaCompiler/parser.hpp"
...
namespace yy {
#line 198 "/home/wowatka/workdir/MRA/JavaCompiler/parser.hpp"
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // declarations
      char dummy3[sizeof (DeclarationList*)];

      // expr
      char dummy4[sizeof (Expression*)];

      // exprs
      char dummy5[sizeof (ExpressionList*)];

      // field_declaration
      char dummy6[sizeof (FieldDeclaration*)];

      // field_invocation
      char dummy7[sizeof (FieldInvokeExpression*)];

      // formals
      char dummy8[sizeof (FormalsList*)];

      // local_variable_declaration
      char dummy9[sizeof (LocalVarDeclaration*)];

      // main_class
      char dummy10[sizeof (MainClass*)];

      // method_declaration
      char dummy11[sizeof (MethodDeclaration*)];

      // method_invocation
      char dummy12[sizeof (MethodInvocation*)];

      // program
      char dummy13[sizeof (Program*)];

      // statement
      char dummy14[sizeof (Statement*)];

      // statements
      char dummy15[sizeof (StatementList*)];
      ...
      // declaration
      char dummy19[sizeof (std::variant<FieldDeclaration*, MethodDeclaration*>)];
    };
    ...
    TOK_BITWISEOR = 266,           // "||"
    TOK_BITWISEAND = 267,          // "&&"
    TOK_LEFTFBRACKET = 268,        // "{"
    TOK_RIGHTFBRACKET = 269,       // "}"
    TOK_LBRACKET = 270,            // "("
    TOK_RBRACKET = 271,            // ")"
    TOK_COMMA = 272,               // ","
    TOK_SEMICOLON = 273,           // ";"
    TOK_LEFTSBRACKET = 274,        // "["
    TOK_RIGHTSBRACKET = 275,       // "]"
    TOK_EQUALS = 276,              // "="
    TOK_DOT = 277,                 // "."
    TOK_BANG = 278,                // "!"
    TOK_CLASS = 279,               // "class"
    TOK_EXTENDS = 280,             // "extends"
    TOK_PUBLIC = 281,              // "public"
    TOK_INT = 282,                 // "int"
    TOK_BOOLEAN = 283,             // "boolean"
    TOK_VOID = 284,                // "void"
    TOK_ASSERT = 285,              // "assert"
    TOK_IF = 286,                  // "if"
    TOK_WHILE = 287,               // "while"
    TOK_ELSE = 288,                // "else"
    TOK_PRINT = 289,               // "System.out.println"
    TOK_RETURN = 290,              // "return"
    TOK_THISINV = 291,             // "this."
    TOK_THIS = 292,                // "this"
    TOK_LENGTH = 293,              // "length"
    TOK_NEW = 294,                 // "new"
    TOK_TRUE = 295,                // "true"
    TOK_FALSE = 296,               // "false"
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_PUBLIC = 26,                           // "public"
        S_INT = 27,                              // "int"
        S_BOOLEAN = 28,                          // "boolean"
        S_VOID = 29,                             // "void"
        S_ASSERT = 30,                           // "assert"
        S_IF = 31,                               // "if"
        S_WHILE = 32,                            // "while"
        S_ELSE = 33,                             // "else"
        S_PRINT = 34,                            // "System.out.println"
        S_RETURN = 35,                           // "return"
        S_THISINV = 36,                          // "this."
        S_THIS = 37,                             // "this"
        S_LENGTH = 38,                           // "length"
        ...
        S_local_variable_declaration = 63,       // local_variable_declaration
        S_method_invocation = 64,                // method_invocation
        S_exprs = 65,                            // exprs
        S_field_invocation = 66,                 // field_invocation
        S_expr = 67                              // expr
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_class_declarations: // class_declarations
      ...
      case symbol_kind::S_field_declaration: // field_declaration
        value.move< FieldDeclaration* > (std::move (that.value));
        break;

      case symbol_kind::S_field_invocation: // field_invocation
        value.move< FieldInvokeExpression* > (std::move (that.value));
        break;

      case symbol_kind::S_formals: // formals
        value.move< FormalsList* > (std::move (that.value));
        break;

      case symbol_kind::S_local_variable_declaration: // local_variable_declaration
        value.move< LocalVarDeclaration* > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_declaration: // declaration
        value.move< std::variant<FieldDeclaration*, MethodDeclaration*> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, FieldDeclaration*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const FieldDeclaration*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, FieldInvokeExpression*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const FieldInvokeExpression*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, FormalsList*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const FormalsList*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, LocalVarDeclaration*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const LocalVarDeclaration*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::variant<FieldDeclaration*, MethodDeclaration*>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::variant<FieldDeclaration*, MethodDeclaration*>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_field_declaration: // field_declaration
        value.template destroy< FieldDeclaration* > ();
        break;

      case symbol_kind::S_field_invocation: // field_invocation
        value.template destroy< FieldInvokeExpression* > ();
        break;

      case symbol_kind::S_formals: // formals
        value.template destroy< FormalsList* > ();
        break;

      case symbol_kind::S_local_variable_declaration: // local_variable_declaration
        value.template destroy< LocalVarDeclaration* > ();
        break;
      ...
      case symbol_kind::S_declaration: // declaration
        value.template destroy< std::variant<FieldDeclaration*, MethodDeclaration*> > ();
        break;
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_BOOLEQUALS (location_type l)
      {
        return symbol_type (token::TOK_BOOLEQUALS, std::move (l));
      }
#else
      static
      symbol_type
      make_BOOLEQUALS (const location_type& l)
      {
        return symbol_type (token::TOK_BOOLEQUALS, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_BITWISEOR (location_type l)
      {
        return symbol_type (token::TOK_BITWISEOR, std::move (l));
      }
#else
      static
      symbol_type
      make_BITWISEOR (const location_type& l)
      {
        return symbol_type (token::TOK_BITWISEOR, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_BITWISEAND (location_type l)
      {
        return symbol_type (token::TOK_BITWISEAND, std::move (l));
      }
#else
      static
      symbol_type
      make_BITWISEAND (const location_type& l)
      {
        return symbol_type (token::TOK_BITWISEAND, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_LEFTFBRACKET (location_type l)
      {
        return symbol_type (token::TOK_LEFTFBRACKET, std::move (l));
      }
#else
      static
      symbol_type
      make_LEFTFBRACKET (const location_type& l)
      {
        return symbol_type (token::TOK_LEFTFBRACKET, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_RIGHTFBRACKET (location_type l)
      {
        return symbol_type (token::TOK_RIGHTFBRACKET, std::move (l));
      }
#else
      static
      symbol_type
      make_RIGHTFBRACKET (const location_type& l)
      {
        return symbol_type (token::TOK_RIGHTFBRACKET, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_LEFTSBRACKET (location_type l)
      {
        return symbol_type (token::TOK_LEFTSBRACKET, std::move (l));
      }
#else
      static
      symbol_type
      make_LEFTSBRACKET (const location_type& l)
      {
        return symbol_type (token::TOK_LEFTSBRACKET, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_RIGHTSBRACKET (location_type l)
      {
        return symbol_type (token::TOK_RIGHTSBRACKET, std::move (l));
      }
#else
      static
      symbol_type
      make_RIGHTSBRACKET (const location_type& l)
      {
        return symbol_type (token::TOK_RIGHTSBRACKET, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_THISINV (location_type l)
      {
        return symbol_type (token::TOK_THISINV, std::move (l));
      }
#else
      static
      symbol_type
      make_THISINV (const location_type& l)
      {
        return symbol_type (token::TOK_THISINV, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_BOYLERPLATE (location_type l)
      {
        return symbol_type (token::TOK_BOYLERPLATE, std::move (l));
      }
#else
      static
      symbol_type
      make_BOYLERPLATE (const location_type& l)
      {
        return symbol_type (token::TOK_BOYLERPLATE, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_field_declaration: // field_declaration
        value.copy< FieldDeclaration* > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_field_invocation: // field_invocation
        value.copy< FieldInvokeExpression* > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_formals: // formals
        value.copy< FormalsList* > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_local_variable_declaration: // local_variable_declaration
        value.copy< LocalVarDeclaration* > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_declaration: // declaration
        value.copy< std::variant<FieldDeclaration*, MethodDeclaration*> > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_field_declaration: // field_declaration
        value.move< FieldDeclaration* > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_field_invocation: // field_invocation
        value.move< FieldInvokeExpression* > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_formals: // formals
        value.move< FormalsList* > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_local_variable_declaration: // local_variable_declaration
        value.move< LocalVarDeclaration* > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_declaration: // declaration
        value.move< std::variant<FieldDeclaration*, MethodDeclaration*> > (YY_MOVE (s.value));
        break;
...
#endif // !YY_YY_HOME_WOWATKA_WORKDIR_MRA_JAVACOMPILER_PARSER_HPP_INCLUDED
